...
class create_dataset_base_dynamic_ELECTRA(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.curriculum_num == 0:
            datapoint = self.datas[idx]
            masked_sentence, label_mask_, lm_position = self.random_masking_(datapoint['encoded_txt'], lm_position, 0.15)
            ...
            if len(find_cpt_list) != 0:
                ...
                mask_prob = float(
                    int(len(datapoint['encoded_txt']) * 0.15) / count_cpttoken_in_example)  # 마스킹 확률 * 전체 찾은 cpt 수 = 전체 토큰 수 *0.15
            ...
            if find_cpt_list != []:
                ...
                for encoded_cpt in encoded_cpt_list:
                    if mask_count > 19:
                        continue
                    cpt_masked_sendtence, mask_count, cpt_mask_tensor, lm_position = self.spaced_cpt_mask(cpt_masked_sendtence,
            ...
            if len(find_cpt_list_3) != 0:
                ...
                for encoded_cpt_in_list in encoded_find_cpt_list:
                    count_cpttoken_in_example += int(len(encoded_cpt_in_list) - 2)
                mask_prob = float(
                    int(len(cpt_masked_sendtence) * 0.10) / count_cpttoken_in_example)  # 마스킹 확률 * 전체 찾은 cpt 수 = 전체 토큰 수 *0.15
            else:
                mask_prob = 0
            ...
            cpt_masked_sendtence, cpt_mask_tensor, lm_position = self.random_masking(cpt_masked_sendtence, cpt_mask_tensor, lm_position, 0.05)
...
class create_dataset_base_dynamic(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.curriculum_num == 0:
            ...
            #total_count = len(label_mask_[label_mask_ == False])
            ...
            if len(find_cpt_list_3) != 0:
                ...
                for encoded_cpt_in_list in encoded_find_cpt_list:
                    count_cpttoken_in_example += int(len(encoded_cpt_in_list) - 2)
                mask_prob = float(
                    int(len(cpt_masked_sendtence) * 0.15) / count_cpttoken_in_example)  # 마스킹 확률 * 전체 찾은 cpt 수 = 전체 토큰 수 *0.15
            else:
                mask_prob = 0
            ...
            #cpt_masked_sendtence, cpt_mask_tensor, lm_position = self.random_masking(cpt_masked_sendtence, cpt_mask_tensor, lm_position, 0.06)

            #total_count = len(cpt_mask_tensor[cpt_mask_tensor == False])
            total_count = len(cpt_mask_tensor) - sum(cpt_mask_tensor)
...
def padded_sequence(samples):
    ...
    for i, LM_example in enumerate(masked_LM):
        if len(LM_example) <= LM_max:
            ...
            #masked_lm_batch.append(LM_example + [tokenizer.pad_token_id] * (LM_max-len(LM_example)))
            #lm_label_batch.append(LM_label[i] + [tokenizer.pad_token_id] * (LM_max-len(LM_label[i])))
            #label_mask_batch.append(label_mask_[i] + [True]*(LM_max-len(label_mask_[i])))
        else:
            masked_lm_batch.append([tokenizer.cls_token_id] + LM_example[:LM_max - 2] + [tokenizer.sep_token_id])
            lm_label_batch.append([tokenizer.cls_token_id] + LM_label[i][:LM_max - 2] + [tokenizer.sep_token_id])
            label_mask_batch.append([True] + label_mask_[i][:LM_max - 2] + [True])
